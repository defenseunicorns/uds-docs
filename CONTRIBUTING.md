...
3. **Add a clone step to `scripts/integration-script.sh`** if docs come from an external repo:
   ```bash
   repos=(
     "https://github.com/defenseunicorns/uds-fleet main ./temp/uds-fleet overlay"
   )
   ```
...
| `scripts/discover-versions.mjs` | Reads `PRODUCTS`, queries GitHub API for release tags, writes `.versions` JSON. |
| `scripts/integration-script.sh` | Reads `.versions` JSON, clones each archived version's docs into `src/content/docs/{contentDir}/{versionSlug}/`. |
...
| `src/routeData.ts` | Adds `data-pagefind-meta` product tags to pages for potential future Pagefind filtering. |

### Versioning pipeline

When a product has `versioning` configured, the build pipeline:

1. `discover-versions.mjs` fetches release tags from the GitHub repo, deduplicates by minor version (keeping latest patch), skips the current release, and writes `.versions`.
2. `integration-script.sh` reads `.versions`, clones each tag, and copies docs into versioned content directories (e.g. `src/content/docs/v0-61/` for Core, `src/content/docs/fleet/v1-2/` for Fleet).
