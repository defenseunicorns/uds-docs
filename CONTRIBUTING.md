...
## Content Structure
- Place docs under `src/content/docs/` using folders that match your product's `sidebarOrder` in `src/products.json`.
- Only directories listed in `sidebarOrder` appear in the sidebar. If `sidebarOrder` is omitted, directories are auto-discovered alphabetically (dot-dirs and version dirs excluded).
...
1. **Add an entry to `src/products.json`**:
   ...
   ```
   - `id` — unique identifier, used internally for sidebar topics and version keys. **Avoid hyphens** — the id is used as a shell variable name suffix for env overrides (`VERSIONS_{id}`) and hyphens are invalid in shell variable names.
   ...
   - `sidebarOrder` — ordered list of sidebar sections. Each entry is either a directory name string (label auto-generated via title-casing) or an object `{ "dir": "...", "label": "..." }` for custom labels. Only listed directories appear — dot-dirs, version dirs, etc. are automatically excluded. If omitted, directories are auto-discovered alphabetically.
...
2. **Create a product landing page** at `src/content/docs/{contentDir}/index.mdx`:
   ...
   - Organize content into subdirectories that match the `sidebarOrder` entries (e.g. `getting-started/`, `concepts/`, `reference/`).
