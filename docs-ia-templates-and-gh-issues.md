...
This document provides templates for common documentation page types in UDS Core docs. Each template includes a GitHub issue for implementing the template as an ADR or guide that writers can reference.
...
### GitHub Issue for How-To Template

**Title:** Create How-To Guide Template ADR

**Labels:** `documentation`, `adr`, `templates`

**Description:**

**Context:**
We need a consistent template for How-To Guides to ensure all task-oriented documentation follows the same structure and provides a complete user experience.

**Objective:**
Create an ADR documenting the How-To Guide template that all documentation writers should follow when creating new task-oriented guides.

**Template Location:**
`docs/adr/XXX-how-to-guide-template.md` or `docs/writing-guides/how-to-guide-template.md`

**Required Content:**

1. **When to Use This Template**
   - For task-oriented documentation
   - When documenting "How to do X"
   - Not for conceptual explanations or reference material

2. **Template Structure**
   - What You'll Accomplish
   - Prerequisites
   - Steps (with expected output)
   - Verification (with success criteria)
   - Troubleshooting (common issues)
   - Next Steps (related content)

3. **Writing Guidelines**
   - Use imperative voice ("Configure the gateway" not "Configuring the gateway")
   - Include working code examples
   - Show expected output for commands
   - Provide verification steps
   - Link to related Concepts and Reference docs
   - Keep scope narrow (single task per guide)

4. **Version-Specific Content**
   - How to handle version-specific guides
   - Banner format for version applicability
   - When to split guides by version

5. **Examples**
   - Include 2-3 example guides using the template
   - Show good vs. bad implementations

**Acceptance Criteria:**

- [ ] ADR created in `docs/adr/` or `docs/writing-guides/`
- [ ] Template includes all required sections
- [ ] Writing guidelines documented
- [ ] Examples provided
- [ ] Template is referenced in contributor documentation
- [ ] Existing How-To guides link to template as reference
...
### GitHub Issue for Troubleshooting Template

**Title:** Create Troubleshooting Runbook Template ADR

**Labels:** `documentation`, `adr`, `templates`, `operations`

**Description:**

**Context:**
Troubleshooting documentation needs consistent structure to help users quickly diagnose and resolve issues. Runbooks should follow a standard format for symptoms → diagnosis → solutions.

**Objective:**
Create an ADR documenting the Troubleshooting Runbook template for all operations troubleshooting documentation.

**Template Location:**
`docs/adr/XXX-troubleshooting-runbook-template.md` or `docs/writing-guides/troubleshooting-runbook-template.md`

**Required Content:**

1. **When to Use This Template**
   - For operational troubleshooting guides
   - When documenting common problems and solutions
   - For both symptom-based and feature-based troubleshooting

2. **Template Structure**
   - Symptoms (what users observe)
   - Common Causes (root cause categories)
   - Diagnostic Steps (how to identify cause)
   - Solutions (step-by-step fixes per cause)
   - Verification (how to confirm resolution)
   - Prevention (avoiding issue in future)

3. **Writing Guidelines**
   - Start with observable symptoms (what user sees)
   - Provide exact error messages when possible
   - Include diagnostic commands with expected output
   - Offer solutions for each common cause
   - Link to related How-To and Concepts
   - Include "Additional Help" section for escalation

4. **Organization**
   - Symptom-based vs. feature-based runbooks
   - When to split vs. combine issues
   - Cross-referencing between runbooks

5. **Examples**
   - 2-3 complete runbook examples
...
- [ ] Writing guidelines documented
- [ ] Examples provided
- [ ] Runbook organization guidance included
- [ ] Template referenced in contributor docs

**Related:**
- Template 1: How-To Guide Template
- Operations & Maintenance section structure
...
### GitHub Issue for Upgrade Guide Template

**Title:** Create Version-Specific Upgrade Guide Template ADR
...
**Context:**
Each UDS Core release needs version-specific upgrade documentation. A consistent template ensures users get the information they need for smooth upgrades.

**Objective:**
Create an ADR documenting the template for version-specific upgrade guides.

**Template Location:**
`docs/adr/XXX-version-upgrade-guide-template.md`

**Required Content:**

1. **When to Create Upgrade Guide**
   - For each minor/major version release
   - When breaking changes exist
   - Not needed for patch releases (unless critical changes)

2. **Template Structure**
   - What's New (features, improvements, fixes)
   - Breaking Changes (with migration examples)
   - Compatibility (Kubernetes, CLI, dependencies)
...
3. **Writing Guidelines**
   - Release date and difficulty rating at top
   - Breaking changes prominently featured
   - Working examples for migrations
   - Clear step-by-step procedures
   - Rollback plan always included
   - Troubleshooting for common issues

4. **Content Sources**
   - How to extract info from release notes
   - Working with changelog data
   - Testing upgrade procedures

5. **Examples**
   - Complete upgrade guide example
...
- [ ] ADR created and approved
- [ ] Template documented
- [ ] Guidelines for writing upgrade guides
- [ ] Examples provided
- [ ] Process for creating guides documented
...
### GitHub Issue for CRD Reference Template

**Title:** Create CRD Reference Page Template ADR

**Labels:** `documentation`, `adr`, `templates`, `reference`

**Description:**

**Context:**
UDS Core defines several CRDs (Package, Exemption, ClusterConfig). These need consistent reference documentation that's both comprehensive and usable.

**Objective:**
Create template for CRD reference pages that balances completeness with readability.

**Template Location:**
`docs/adr/XXX-crd-reference-template.md`

**Required Content:**

1. **When to Use This Template**
   - For all UDS custom resource definitions
   - Both stable and alpha API versions
   - Updated when CRD schema changes

2. **Template Structure**
   - Overview (what it is, when to use)
   - Schema reference table (all fields)
   - Field details (complex fields explained)
   - Complete examples (common use cases)
   - Validation rules
   - Lifecycle behavior
   - Best practices
...
3. **Writing Guidelines**
   ...
   - Explain field interactions and constraints
   - Document default behaviors
   - Include validation error messages
   - Link to task-oriented How-To guides
   - Keep reference factual, put "how-to" in guides

4. **Schema Documentation**
   - How to extract schema from CRD
   - Formatting conventions
   - Documenting nested fields
   - Handling optional vs. required

5. **Examples**
   - Complete Package CR reference example
   - Show simple vs. complex examples

**Acceptance Criteria:**

- [ ] ADR created
- [ ] Template documented
- [ ] Guidelines for extracting schema
- [ ] Example reference page created
- [ ] Integration with generated docs (if applicable)
...
1. **How-To Guide Template** - Task-oriented documentation
2. **Troubleshooting Runbook Template** - Problem diagnosis and resolution
3. **Version-Specific Upgrade Guide Template** - Per-version upgrade docs
4. **CRD Reference Page Template** - Custom resource documentation

### Implementation Plan

1. Create ADRs for each template
2. Add templates to `docs/templates/` or `docs/writing-guides/`
3. Create example documentation using each template
4. Update contributor guide to reference templates
5. Link templates from documentation contribution guidelines

### Template Storage

**Option A:** ADRs (Architecture Decision Records)
```
docs/adr/
├── XXX-how-to-guide-template.md
├── XXX-troubleshooting-runbook-template.md
├── XXX-version-upgrade-guide-template.md
└── XXX-crd-reference-template.md
```

**Option B:** Writing Guides
```
docs/writing-guides/
├── how-to-guide-template.md
├── troubleshooting-runbook-template.md
├── version-upgrade-guide-template.md
└── crd-reference-template.md
```

**Recommendation:** Use **ADRs** because:
- Documents the decision to use standard templates
- Explains rationale for structure choices
- Provides context for future maintainers
- Standard practice for architectural decisions
